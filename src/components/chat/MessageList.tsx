...
function getToolLabel(toolName: string, args: { path?: string; command?: string }): string {
  const fileName = args?.path ? args.path.split("/").pop() : undefined;

  if (toolName === ToolName.StrReplaceEditor) {
    switch (args?.command) {
      case EditorCommand.Create:
        return fileName ? `Creating ${fileName}` : "Creating file";
      case EditorCommand.StrReplace:
      case EditorCommand.Insert:
        return fileName ? `Editing ${fileName}` : "Editing file";
      case EditorCommand.View:
  ...
  if (toolName === ToolName.FileManager) {
    switch (args?.command) {
      case FileManagerCommand.Rename:
        return "Renaming file";
      case FileManagerCommand.Delete:
